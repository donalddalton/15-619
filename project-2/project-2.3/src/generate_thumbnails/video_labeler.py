...
def img_input_handler(event, context):
    ...
    for record in event['Records']:
        ...
        for message in json.loads(record['Sns']['Message'])['Records']:
            ...
            filename = os.path.join(download_path, prefix + '_%d.png')
            ff = FFmpeg(executable=ffmpeg,
                        inputs={video_path: None},
                        outputs={filename: ['-y', '-vf', 'fps=1']})
            ...
            for file in os.listdir(download_path):
                if file.endswith('.png'):
                    ...
                    s3.meta.client.upload_file(upload_path, 'mythumbnailbucket', file)
